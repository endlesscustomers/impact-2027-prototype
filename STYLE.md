...
- **Comparison table** `Compare` component, `.compare` (2026-09-20): humaninterest.com's "us vs the old way" table. A real `<table>`, 1100px, centred: row labels in the first column (`--ink-2`, 600), our column on a continuous `--fill` panel with 24px corners and its name in the accent, the other option in `--ink-2` at 400 with `strong` for a bold lead (the `.regular` rule). Column names at `--fs-body`, never smaller or uppercase. Hairlines between rows cross the panel. A cell's only link is a standalone `a.more` on the site-wide link rule. Under 760px each row becomes a block: the label as its heading in `--ink`, our cell as a 16px-corner `--fill` card with the column name repeated above it in the accent, the other cell plain with its name in `--ink-3`; the header row stays for screen readers. Every claim comes from the `/compare`, `/why-impact`, and `/guarantee` outlines.
