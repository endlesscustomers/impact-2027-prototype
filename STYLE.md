...
- **Comparison table** `Compare` component, `.compare` (2026-09-20): humaninterest.com's "us vs the old way" table. A real `<table>`, 1100px, centred: row labels in the first column (`--ink-2`, 600), our column on a continuous `--fill` panel with 24px corners and its name in the accent, the other option in `--ink-2` at 400 with `strong` for a bold lead (the `.regular` rule). Column names at `--fs-body`, never smaller or uppercase. Hairlines in `--line-strong` between rows, so they read across the panel (Bob: the `--line` ones vanished on it); cells padded 16 x 24 (Bob: compact). A cell's only link is a standalone `a.more` on the site-wide link rule. Under 760px each row becomes a block: the label as its heading in `--ink`, our cell as a 16px-corner `--fill` card with the column name repeated above it in the accent, the other cell plain with its name in `--ink-3`; the header row stays for screen readers. Every claim comes from the `/compare`, `/why-impact`, `/guarantee`, and `/deep-diagnostic` outlines.
- **Tab bar** `Tabs` component, `.tabbar` + `.tabpanel` (2026-09-20): humaninterest.com's category switcher. A 1000px `--fill` pill track, 6px padding; tabs at `--fs-ui` / `--w-ui` in `--ink`, equal width, the selected one a `--bg-panel` pill with `--shadow-bar` and its name in the accent. Real `role="tab"` buttons with arrow-key movement. Panels are `.tabpanel[data-panel]` siblings, each with a centred H3 (`--fs-h3`), an 18px regular `.sub` in `--ink-2`, then the module they hold; the script toggles `hidden`. Under 760px the track becomes one full-width pill showing the current tab (accent, chevron) that opens a `--bg-panel` list, as Human Interest's does on a phone. Labels are plain text.
- **Spotlight slider** `Spotlight` component, `.spotlight` (2026-09-20): humaninterest.com's home-page testimonial. A 1000px `--bg-panel` card with 24px corners: the photo fills the left column edge to edge (`object-fit: cover`, 520px minimum), the right column holds a quote mark in the heading font at `--fs-h2`, the quote at `--fs-lg` / `--w-para` / `--ink`, the person (`b` name, `--ink-2` title at company, `--ink-3` meta line of facts the story records), and `Read the Story →` as `a.more`. 48px outline arrows sit outside the card at mid-height from 1200px, under it beside the dots below that; the selected dot stretches to a 40px accent pill. Under the dots, one centred `See All Success Stories →` on the link rule and an optional fine-print `.note`. Under 760px the photo stacks above the text at 3:2. Quotes verbatim and checked against the banned list; photo, meta, and link come from `stories.ts`.
